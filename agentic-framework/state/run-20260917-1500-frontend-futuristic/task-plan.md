# Task Plan — 20260917-1500-frontend-futuristic

## Metadados
- **run_id:** 20260917-1500-frontend-futuristic
- **Data de abertura:** 2026-09-17
- **Solicitado por:** Alex — design muda 100%, organização e componentes futuristas, estrutura funcional preservada, 5 skills
- **Spec de origem:** BRD_portal_noticias_versao_1.docx + agentic-framework/specs/* (13 specs) + .claude/skills/frontend-portal/SKILL.md

## Objetivo
Recriar `frontend/` do zero com design completamente novo e futurista (bento-grid/HUD, vidro/neon, papel escuro, organização reposicionada), mantendo 100% da estrutura funcional (31 rotas + contratos lib/api.ts + SEO/LGPD/skip-link/anti-flash) e evidenciando as 5 skills em runtime.

## Escopo
### Dentro do escopo
- Scaffold futurista: `tailwind.config.ts` theme.extend mapeando tokens futuristas `--cor-*`, `--espaco-*`, `--raio-*`, `--sombra-*`, `--z-*` para `var(--cor-*)`; `postcss.config.js`; `components.json` new-york rsc tsx cssVariables; `lib/utils.ts` cn; `app/globals.css` com `@tailwind` + `@layer base` (tokens futuristas + Inter/Source_Serif_4 via next/font + data-theme + prefers-reduced-motion) + bento, vidro, néon contido
...
- Páginas núcleo (D1): `app/page.tsx` (hero protagonista futurista + rio bento-grid/HUD + HomeHero/HomeSidebar/BlocoEditoria/MaisLidas/CartaoEsqueleto), `app/comunidade/**` (feed/nova/[id]), `app/radar/**`, `app/planos/**`, `app/noticia/cluster|item/[id]`, `app/autor/[id]`
- Páginas auth/admin/apoio (D2): `app/cadastro|login|onboarding|minha-conta|lista-de-espera|jornalista/**|admin/**|empresa|paginas/[slug]|privacidade/**|recuperar|redefinir|verificar|not-found` (31 rotas preservadas, visual novo, organização reposicionada)
- Design-spec futurista 2 passes (paper #FDFBF7 / ink #0B0B1A / signal #FF2E2E + néon ciano/violeta, bento-grid, vidro, HUD)
...
- Direção paper/ink+signal aprovada (paper #FDFBF7, ink #0B0B1A, signal #FF2E2E, HUD) + complemento futurista (néon ciano #06B6D4 / violeta #7C3AED, vidro, bento, papel escuro) — motivo: contraste editorial + sinal urgente sem cair no creme/terracota genérico; ousadia contida
- Futurista = HUD/bento/vidro/neon contido, não cyberpunk carregado — 1 elemento ousado (hero holográfico), resto disciplinado
...
- Tokens SOMENTE var(--cor-*) etc. via tailwind theme.extend; nunca hex hardcoded fora de globals.css
...
- Design futurista: bento-grid, HUD, vidro/neon contido, organização reposicionada (ex.: métricas em bento, feed em HUD), motion só transform/opacity + prefers-reduced-motion
...
1. Design 100% novo futurista evidenciado (bento/HUD/vidro/neon contido, organização reposicionada) e não-genérico (revisão anti-genérico registrada)
