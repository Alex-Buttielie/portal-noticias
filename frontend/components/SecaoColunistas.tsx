...
// ---------------------------------------------------------------------------
// Seção Colunistas na Home (FRENTE 2) — mesmo padrão visual dos cards de
...
export function SecaoColunistas() {
  const [colunistas, setColunistas] = useState<Colunista[] | null>(null);

  useEffect(() => {
    let vivo = true;
    (async () => {
      const lista = await carregarColunistas(4);
      if (vivo) setColunistas(lista);
    })();
    return () => {
      vivo = false;
    };
  ...
  if (colunistas !== null && colunistas.length === 0) return null;
  ...
  return (
    <section aria-label="Colunistas" className="space-y-3">
      ...
      {!colunistas ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Carregando colunistas">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-64 animate-pulse rounded-[var(--raio-lg)] border border-[var(--cor-borda)] bg-[var(--cor-fundo-elevado)]"
            />
          ))}
        </div>
      ) : (
