...
export default function Page(){
  ...
  const carregar=async()=>{ setErr(null); setLoading(true); try{ const r=await api.adminListarAssinaturas(token||"",{search:busca||undefined}); setItens(r.results||[]);}catch(e:any){ setErr(e?.message||"API offline - mock"); setItens([{id:1,user_email:"user@exemplo.com",user_nome:"User Mock",plan:{id:2,nome:"Premium",preco:"29.90",duracao_dias:30},status:"ativa",preco_cobrado:"29.90",criado_em:new Date().toISOString()}]);} finally{ setLoading(false); } };
