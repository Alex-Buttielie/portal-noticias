...
export function AcoesNoticia({ entrada }: { entrada: FeedEntrada }) {
  ...
  function onSave() { const n = alternarSalvo(entrada); setSalvo(n); if(n) setSaveOpen(true); }
  async function copiar(){ try{ await navigator.clipboard.writeText(url); setCopiado(true); setTimeout(()=>setCopiado(false),2000);}catch{} }
  async function nativo(){ const data={title:entrada.titulo,text:entrada.resumo,url}; try{ if(navigator.share) await navigator.share(data); else await copiar(); }catch{} }
        ...
        <DialogContent className="border-[var(--cor-borda)] bg-[var(--cor-fundo-card)]">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <a href={`https://wa.me/?text=${encTitulo}%20${encUrl}`} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-[44px] items-center justify-center rounded-md border border-[var(--cor-borda)] bg-[var(--cor-fundo-card)] px-3 text-sm font-medium text-[var(--cor-texto)] hover:bg-[var(--cor-fundo-elevado)]">WhatsApp</a>
              <a href={`https://twitter.com/intent/tweet?text=${encTitulo}&url=${encUrl}`} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-[44px] items-center justify-center rounded-md border border-[var(--cor-borda)] bg-[var(--cor-fundo-card)] px-3 text-sm font-medium text-[var(--cor-texto)] hover:bg-[var(--cor-fundo-elevado)]">X / Twitter</a>
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${encUrl}`} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-[44px] items-center justify-center rounded-md border border-[var(--cor-borda)] bg-[var(--cor-fundo-card)] px-3 text-sm font-medium text-[var(--cor-texto)] hover:bg-[var(--cor-fundo-elevado)]">Facebook</a>
            </div>
