...
import { obterDetalheCluster, obterFeed, ApiError, type FeedDetalhe } from "@/lib/api";
...
function classificar(erro: unknown): { tipo: "inexistente" } | { tipo: "indisponivel"; requestId: string | null; mensagem: string } {
  ...
  return {
    ...
    mensagem: erro instanceof Error ? erro.message : "Falha inesperada ao consultar o cluster.",
  };
}

async function getD(id:string): Promise<ResultadoDetalhe>{
  try{ return { tipo: "detalhe", dados: await obterDetalheCluster(id) }; }
  catch(erro){ return classificar(erro); }
}
export async function generateMetadata({params}:{params:{id:string}}): Promise<Metadata>{ let t=`Cluster #${params.id} - ${SITE_NAME}`; try{ const r=await getD(params.id); if(r.tipo==="detalhe") t=r.dados.titulo; }catch{ /* 404/indisponivel tratados abaixo */ } return {title:t, openGraph:{url:`${SITE_URL}/noticia/cluster/${params.id}`}}; }
...
export default async function Page({params}:{params:{id:string}}){
  ...
  }catch{ /* relacionados são opcionais; o cluster continua válido */ }
  return (<><script type="application/ld+json" dangerouslySetInnerHTML={{__html: JSON.stringify(jsonLd)}} /><NoticiaReporter entryTipo="cluster" entryId={d.id} categoria={d.categoria} /><LeituraPremium detalhe={d} relacionados={relacionados} heroSrc={heroSrc} imagemReal={imagemReal} /><div className="mx-auto mt-6 max-w-3xl px-4"><CoberturaCompleta tipo="cluster" id={d.id} /><div className="mt-6 flex flex-wrap gap-2"><Button asChild variant="outline" className="min-h-[44px] border-[var(--cor-borda)]"><Link href="/">Voltar ao início</Link></Button><Button asChild variant="outline" className="min-h-[44px] border-[var(--cor-borda)]"><Link href="/arquivo">Ver arquivo</Link></Button></div></div></>);
}
