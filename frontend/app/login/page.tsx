...
export default function Page(){
  ...
  const onSubmit=async(d:{email:string;senha:string})=>{ setErro(null); try{ await fazerLogin(d.email,d.senha); let u:any=null; try{ const raw=typeof window!=="undefined"? window.localStorage.getItem("portal_noticias_usuario"):null; if(raw) u=JSON.parse(raw);}catch{} if(u?.deve_trocar_senha){ r.push("/trocar-senha"); return; } if(u && !u.onboarding_concluido){ setDlgOnboarding(true); return; } if(u?.papel==="admin"){ setDlgAdmin(true); return; } r.push("/minha-conta"); }catch(e:any){ setErro(e?.message||"Falha no login."); } };
