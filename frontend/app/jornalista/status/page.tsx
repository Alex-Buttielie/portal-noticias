...
export default function Page(){
  ...
  return (<div className="mx-auto max-w-2xl space-y-4 py-6"><div className="hud-line" aria-hidden />
    <Card className="bento border-[var(--cor-borda)] bg-[var(--cor-fundo-card)]"><CardHeader><CardTitle>Status do credenciamento</CardTitle><CardDescription className="text-[var(--cor-texto-suave)]">Acompanhe sua solicitação</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      {solic? (<div className="rounded-[var(--raio-md)] border border-[var(--cor-borda)] bg-[var(--cor-fundo-elevado)] p-3"><p className="text-sm text-[var(--cor-texto)]">Solicitacao #{solic.id} - <Badge className="bg-[var(--cor-primaria)] text-[var(--cor-texto-invertido)]">{solic.status}</Badge></p><p className="text-xs text-[var(--cor-texto-suave)]">{solic.cidade}/{solic.uf} - {formatarDataCurta(solic.criado_em)}</p>{solic.motivo_decisao&&<p className="mt-2 text-sm text-[var(--cor-texto-suave)]">{solic.motivo_decisao}</p>}</div>) : <p className="text-sm text-[var(--cor-texto-suave)]">Nenhuma solicitacao encontrada - <a href="/jornalista/solicitar" className="text-[var(--cor-primaria)] underline">solicitar</a>.</p>}
