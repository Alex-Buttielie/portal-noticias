...
(`mock.patch` no alvo `catalogo_noticias.providers.summarization.requests.post`)
e nenhuma credencial real e usada. O caminho feliz e testado com um dublê de
resposta HTTP valida, o que prova que o provedor EXTERNAL e de fato chamado
quando tudo funciona — sem esse teste a suite passaria mesmo com o provedor
...
class TestFallbackExecuta:
    def test_sem_credencial_configurada_usa_fallback_local_e_registra_metrica(self):
        ...
        with patch("requests.post", side_effect=AssertionError("rede nao pode ser chamada")):
    ...
    def test_timeout_do_provedor_usa_fallback_local_e_registra_metrica(self):
        ...
        with patch("requests.post", side_effect=requests.Timeout("estourou o prazo")):
            _roda_ingestao(provider)
    ...
    def test_erro_500_do_provedor_usa_fallback_local_com_motivo_erro_http(self):
        ...
        with patch("requests.post", return_value=_resposta_http("{}", status_code=500)):
            _roda_ingestao(provider)
    ...
    def test_rate_limit_429_tem_motivo_proprio_distinto_de_erro_http(self):
        ...
        with patch("requests.post", return_value=_resposta_http("{}", status_code=429)):
            _roda_ingestao(provider)
    ...
    def test_falha_de_conexao_usa_fallback_com_motivo_erro_http(self):
        ...
        with patch("requests.post", side_effect=requests.ConnectionError("recusada")):
            _roda_ingestao(provider)
    ...
    def test_resposta_malformada_do_provedor_usa_fallback_com_motivo_resposta_invalida(self):
        ...
        with patch("requests.post", return_value=_resposta_http("isto nao e json")):
            _roda_ingestao(provider)
    ...
    def test_timeout_configurado_invalido_e_corrigido_para_nao_haver_chamada_sem_limite(self):
        ...
        for indice, valor_invalido in enumerate((0, -5, None, "abc")):
            ...
            with patch("requests.post", return_value=_resposta_http(_resposta_lote([(1, "R.", "geral")]))) as mock_post:
...
class TestFallbackPublica:
    def test_item_do_fallback_entra_no_feed_publico_como_noticia_completa(self):
        ...
        with patch("requests.post", side_effect=AssertionError("rede")):
            _roda_ingestao(provider)
    ...
    def test_sem_manchete_o_fallback_sinaliza_e_nao_publica(self):
        ...
        with patch("requests.post", side_effect=requests.Timeout("prazo")):
...
class TestLog:
    ...
    def test_log_do_timeout_traz_o_motivo_e_a_excecao(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            with patch("requests.post", side_effect=requests.Timeout("prazo estourado")):
                _roda_ingestao(provider)
    ...
    def test_log_nao_contem_o_valor_da_credencial(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            for cenario in cenarios:
                with patch("requests.post", **cenario):
...
class TestContratoDoProvider:
    def test_resposta_malformada_ainda_levanta_summarizationprovidererror(self):
        ...
        provider = LLMHttpSummarizationProvider(api_key=CHAVE_FICTICIA)
        with patch("requests.post", return_value=_resposta_http("formato invalido")):
